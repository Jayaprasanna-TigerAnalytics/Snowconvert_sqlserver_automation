...
CREATE VIEW vw_ActiveInventory WITH SCHEMABINDING AS
...
CREATE VIEW vw_RecentCustomerOrders WITH SCHEMABINDING AS
...
CREATE VIEW vw_SupplierPerformance WITH SCHEMABINDING AS
SELECT SupplierID, COUNT_BIG(PurchaseOrderID) AS TotalOrders,
...
CREATE VIEW vw_LowStockItems WITH SCHEMABINDING AS
...
CREATE VIEW vw_OrderFulfillmentRate WITH SCHEMABINDING AS
SELECT CustomerID, COUNT_BIG(OrderID) AS TotalOrders,
...
CREATE VIEW vw_MonthlySalesSummary WITH SCHEMABINDING AS
SELECT YEAR(OrderDate) AS Year, MONTH(OrderDate) AS Month,
       SUM(TotalAmount) AS TotalSales, COUNT_BIG(OrderID) AS TotalOrders
FROM dbo.Orders
...
CREATE VIEW vw_ShipmentTracking WITH SCHEMABINDING AS
SELECT ShipmentID, OrderID, Carrier, TrackingNumber, ShipmentDate, EstimatedDeliveryDate, Status
FROM dbo.Shipments;
...
CREATE VIEW vw_ProductDemand WITH SCHEMABINDING AS
SELECT ProductID, COUNT_BIG(OrderID) AS OrdersCount, SUM(Quantity) AS TotalQuantityOrdered
...
CREATE VIEW vw_HighValueCustomers WITH SCHEMABINDING AS
...
CREATE VIEW vw_UnshippedOrders WITH SCHEMABINDING AS
...
CREATE VIEW vw_OrderFulfillmentRate AS
...
SELECT FORMAT(OrderDate, 'yyyy-MM') AS Month, SUM(TotalAmount) AS TotalSales, COUNT(OrderID) AS TotalOrders
FROM Orders
...
CREATE VIEW vw_ProductDemand AS
SELECT ProductID, ProductName, COUNT(OrderID) AS OrdersCount, SUM(Quantity) AS TotalQuantityOrdered
FROM OrderDetails
...
CREATE VIEW vw_UnshippedOrders AS
SELECT OrderID, CustomerID, OrderDate, Status
FROM Orders
WHERE Status NOT IN ('Shipped', 'Delivered');
