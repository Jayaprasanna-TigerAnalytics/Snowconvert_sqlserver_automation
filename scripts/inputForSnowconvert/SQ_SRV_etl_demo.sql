...
WHEN MATCHED THEN 
    UPDATE SET target.Quantity = source.Quantity, target.LastUpdated = GETDATE()
...
WHEN MATCHED THEN 
    UPDATE SET target.Status = source.Status, target.LastUpdated = GETDATE()
WHEN NOT MATCHED THEN 
    INSERT (OrderID, SupplierID, OrderDate, Status, LastUpdated)
    VALUES (source.OrderID, source.SupplierID, source.OrderDate, source.Status, GETDATE());
...
WHEN MATCHED THEN 
    UPDATE SET target.Status = source.Status, target.ETA = source.ETA
WHEN NOT MATCHED THEN 
